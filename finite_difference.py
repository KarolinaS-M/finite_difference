...
• forward difference → possible divergence for large $h$  
...
axes[0].plot(t, x_backward, "o--", label="Backward FD")
...
axes[1].plot(t, x_central, "o--", label="Central FD")
...
axes[2].plot(t, x_forward, "o--", label="Forward FD")
...
st.info(
    "As the grid becomes coarser (larger step size h), the forward scheme "
    "may overshoot and diverge, while the central scheme introduces spurious "
    "oscillations. The backward method remains stable for all tested resolutions, "
    "illustrating its importance in stiff and dissipative systems."
)